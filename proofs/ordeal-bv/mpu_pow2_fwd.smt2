; gale#173 pilot 2 — src/mpu.rs:98 is_power_of_two, FORWARD direction of the by(bit_vector) iff.
;   (n>0 AND n&(n-1)==0)  =>  n in {1,2,...,2^31}
; Refute (idiom AND NOT enumeration) via implicitly-conjoined asserts. UNSAT => forward holds for all n:u32.
(set-logic QF_BV)
