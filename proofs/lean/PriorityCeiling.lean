...
structure SystemConfig where
  ...
  ceilings_valid : ∀ r : Resource, r ∈ resources →
    ∀ cs : CriticalSection, cs ∈ sections →
...
theorem pcp_bounded_blocking (cfg : SystemConfig) (t : PTask) :
    ∀ cs, cs ∈ blockingSections cfg t → cs.duration ≤ maxBlockingTime cfg t := by
  intro cs hcs
...
def pcpAllows (currentCeilings : List Priority) (attempt : AcquisitionAttempt) : Prop :=
  ∀ c, c ∈ currentCeilings → attempt.task.priority > c
...
/-- A wait-for graph is a list of edges. Using abbrev so List instances propagate. -/
abbrev WaitGraph := List WaitEdge
...
inductive Reachable (edges : WaitGraph) : Priority → Priority → Prop where
  | refl (p : Priority) : Reachable edges p p
  | step (p q r : Priority) :
      (∃ e, e ∈ edges ∧ e.waiter.priority = p ∧ e.holder.priority = q) →
      Reachable edges q r →
      Reachable edges p r
...
theorem reachable_priority_nondecreasing (edges : WaitGraph)
    (h_pcp : ∀ e, e ∈ edges → e.waiter.priority < e.holder.priority)
  ...
  | step p' q' r' hedge _ ih =>
    ...
    exact Nat.le_trans (Nat.le_of_lt hlt) ih
...
theorem pcp_no_deadlock (edges : WaitGraph)
  ...
  exact absurd hpq (Nat.not_lt.mpr hqp)
...
theorem priority_inversion_bounded (inv : PriorityInversion)
    (cs : CriticalSection)
    (_h_holds : cs.task = inv.low)
    (_h_res : cs.resource = inv.resource) :
...
theorem pcp_single_blocking_per_access (cfg : SystemConfig) (t : PTask)
    (_bs : List CriticalSection)
    (_h_bs : _bs = blockingSections cfg t) :
