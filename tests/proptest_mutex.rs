...
proptest! {
    ...
    fn lock_unlock_roundtrip(thread_id in 0u32..1000) {
        let mut m = Mutex::init();
        prop_assert_eq!(m.try_lock(ThreadId { id: thread_id }), LockResult::Acquired);
    ...
    fn contention_preserves_state(
        ...
        let mut m = Mutex::init();
        m.try_lock(ThreadId { id: owner_id });
        prop_assert_eq!(m.try_lock(ThreadId { id: other_id }), LockResult::WouldBlock);
        prop_assert_eq!(m.owner_get(), Some(ThreadId { id: owner_id }));
    ...
    fn non_owner_unlock_error(
        ...
        let mut m = Mutex::init();
        m.try_lock(ThreadId { id: owner_id });
        let result = m.unlock(ThreadId { id: other_id });
    ...
    fn unlock_unlocked_error(thread_id in 0u32..1000) {
        let mut m = Mutex::init();
        let result = m.unlock(ThreadId { id: thread_id });
