...
pub fn partitions_overlap_decide(
    ...
    ensures
        result == ({
            let p = MemPartition { start: part_start, size: part_size, attr: 0 };
            let e = MemPartition { start: existing_start, size: existing_size, attr: 0 };
            p.overlaps_spec(&e)
        }),
{
...
pub fn partition_valid_decide(start: u32, size: u32) -> (result: bool)
    ensures
        result == (MemPartition { start, size, attr: 0 }).is_valid(),
{
