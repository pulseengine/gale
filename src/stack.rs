...
pub fn push_decide(
    ...
    capacity: u32,
    has_waiter: bool,
) -> (result: PushDecideResult)
    requires
        capacity > 0,
        count <= capacity,
    ensures
        has_waiter ==> {
            &&& result.decision === PushDecision::WakeWaiter
            &&& result.new_count == count
        },
        !has_waiter && count < capacity ==> {
            &&& result.decision === PushDecision::Store
            &&& result.new_count == count + 1
        },
        !has_waiter && count >= capacity ==> {
            &&& result.decision === PushDecision::Full
            &&& result.new_count == count
        },
{
    if has_waiter {
...
pub fn pop_decide(
    count: u32,
    is_no_wait: bool,
) -> (result: PopDecideResult)
    requires
        true,
    ensures
        count > 0 ==> {
            &&& result.decision === PopDecision::Pop
            &&& result.new_count == (count - 1) as u32
        },
        count == 0 && is_no_wait ==> {
            &&& result.decision === PopDecision::Busy
            &&& result.new_count == 0
        },
        count == 0 && !is_no_wait ==> {
            &&& result.decision === PopDecision::Pend
            &&& result.new_count == 0
        },
{
    if count > 0 {
