...
impl Tasks {
    ...
    {
        // When the guard is false, self == old(self): deadline array untouched,
        // so every framing postcondition holds pointwise and inv() (state/ready
        // only) is preserved unmodified — no `else` branch needed.
        if h < MAX_TASKS as u32 && matches!(self.state[h as usize], TaskState::Pending) {
            self.deadline[h as usize] = d;
        }
        proof {
            // The deadline write touches neither `state` nor `ready`, so both of
            // inv()'s clauses carry over from `old(self).inv()`. Fire the forall
            // trigger explicitly (via the point-wise lemma) so Verus re-derives it.
            assert(self.ready < 256u32);
            assert forall|i: int| 0 <= i < MAX_TASKS implies
                (#[trigger] self.ready_bit(i)) ==> self.state[i as int] === TaskState::Pending
            by {
                if self.ready_bit(i) {
                    lemma_inv_ready_implies_pending(*old(self), i);
                }
            }
        }
    }
...
mod exec_kani {
    ...
    fn arbitrary_tasks_inv() -> Tasks {
        let state: [TaskState; MAX_TASKS] = [
            arb_state(), arb_state(), arb_state(), arb_state(),
            arb_state(), arb_state(), arb_state(), arb_state(),
        ];
        let prio: [u32; MAX_TASKS] = kani::any();
        ...
        Tasks { state, prio, deadline, ready }
    }
    ...
    fn set_deadline_sets_only_h() {
        ...
        // Snapshot the pre-state fields we frame (Tasks derives no Clone; TaskState
        // is a fieldless enum, so `as u8` captures each slot's discriminant).
        ...
        let h_pending =
            (h as usize) < MAX_TASKS && matches!(t.state[h as usize], TaskState::Pending);
