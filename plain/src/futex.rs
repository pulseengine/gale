...
pub fn wait_decide(val: u32, expected: u32) -> WaitDecision {
    if val == expected { WaitDecision::Block } else { WaitDecision::Mismatch }
}
...
pub fn wake_decide(num_waiters: u32, wake_all: bool) -> WakeDecision {
    if num_waiters == 0 {
        WakeDecision {
            woken: 0,
            remaining: 0,
