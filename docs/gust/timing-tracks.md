...
(`docs/superpowers/plans/2026-07-15-gust-safety-release-line.md`, §3) carries two
...
synth cycle bound (T4)          — sound C_i per poll segment, from the emitted
        ...
        ×
Verus-proven poll structure     — poll_round dispatches each ready task exactly once,
  (src/executor.rs)               run-to-completion, loop machine-checked terminating
                                  (decreases ready_popcount(), ≤ MAX_TASKS = 8)
        ×
