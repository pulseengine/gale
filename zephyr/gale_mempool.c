 ...
 *
 * Gale mempool — Extract/Decide/Apply pattern.
 *
 * This is kernel/mempool.c with alloc/free rewritten to use Rust
 * decision structs.  C extracts kernel state (spinlock, heap pointer
 * storage), Rust decides the action, C applies it.
 *
 * sys_heap internals, alignment, coalescing, and actual memory
 * allocation remain in Zephyr.  k_free delegates to k_heap_free
 * (from gale_kheap.c) which has its own decision struct.
 *
 * Verified operations (Verus proofs):
 *   gale_k_mempool_alloc_decide — MP2 (alloc), MP3 (full)
 *   gale_k_mempool_free_decide  — MP4 (free), MP5 (conservation)
 */
			    ...
			    sys_heap_allocator_t sys_heap_allocator)
{
	...
	/* Decide: Rust determines action based on allocation result */
	struct gale_mempool_alloc_decision d = gale_k_mempool_alloc_decide(
		mem != NULL ? 1U : 0U);

	/* Apply: execute Rust's decision */
	if (d.action == GALE_MEMPOOL_ACTION_RETURN_NULL) {
		return NULL;
	}

	/* RETURN_PTR: store heap reference and return aligned pointer */
	heap_ref = mem;
	*heap_ref = heap;
	mem = ++heap_ref;
