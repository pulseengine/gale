 ...
 *
 * Gale dynamic — phase 2: Extract→Decide→Apply pattern.
 *
 * This is kernel/dynamic.c with alloc/free rewritten to use Rust
 * decision structs.  C extracts kernel state (bitarray, stack pool),
 * Rust decides the action, C applies it.
 *
 * Verified operations (Verus proofs):
 *   gale_dynamic_alloc_decide — DY2 (alloc, active += 1), DY3 (pool full)
 *   gale_dynamic_free_decide  — DY4 (free, no underflow)
 */
...
/* Gale active count — tracks how many pool slots are in use */
static unsigned int gale_pool_active;
...
static k_thread_stack_t *z_thread_stack_alloc_pool(size_t size, int flags)
{
	...
	/* Decide: Rust determines whether pool has room */
	struct gale_dynamic_alloc_decision d = gale_dynamic_alloc_decide(
		gale_pool_active, (uint32_t)CONFIG_DYNAMIC_THREAD_POOL_SIZE);

	if (d.action == GALE_DYNAMIC_ACTION_POOL_FULL) {
		LOG_DBG("unable to allocate stack from pool (Gale: full)");
		return NULL;
	}

	/* Extract: try to allocate from bitarray */
...
int z_impl_k_thread_stack_free(k_thread_stack_t *stack)
{
	...
	if (CONFIG_DYNAMIC_THREAD_POOL_SIZE > 0) {
		if (IS_ARRAY_ELEMENT(dynamic_stack, stack)) {
			...
			/* Decide: Rust determines whether free is valid */
			struct gale_dynamic_free_decision d =
				gale_dynamic_free_decide(gale_pool_active);

			if (d.action == GALE_DYNAMIC_ACTION_FREE_OK) {
				/* Apply: update active count */
				gale_pool_active = d.new_active;
			}
			/* UNDERFLOW: bitarray freed but counter already 0 — logic error */
