 ...
 *
 * Uses Gale verified match check for thread ID compatibility and
 * Gale verified data exchange for size computation.
 *
			       ...
			       struct k_mbox_msg *rx_msg)
{
	...
	if (((tx_msg->tx_target_thread == (k_tid_t)K_ANY) ||
		...
		/* --- Gale verified data exchange size --- */
		rx_msg->size = gale_mbox_data_exchange(
			(uint32_t)tx_msg->size, (uint32_t)rx_msg->size);
