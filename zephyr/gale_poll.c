 ...
 *
 * Gale poll — phase 2: Extract->Decide->Apply pattern.
 *
 * This is kernel/poll.c with k_poll_signal_raise rewritten to use a Rust
 * decision struct. C extracts kernel state (spinlock, dlist side effects),
 * Rust decides signal state mutation, C applies it.
 *
 * The main k_poll() loop and event registration stay in C — the iteration
 * over heterogeneous kernel objects (sem, queue, signal, msgq, pipe) is
 * inherently a C-side concern. Rust decides per-event condition checks
 * via the phase-1 helpers (gale_poll_check_sem, etc).
 *
 * Verified operations (Verus proofs):
 *   gale_poll_event_init            — PL1 (NOT_READY init)
 *   gale_poll_check_sem             — PL3 (SEM_AVAILABLE iff count > 0)
 *   gale_k_poll_signal_raise_decide — PL7 (set signaled + result)
 *   gale_poll_signal_reset          — PL8 (clear signaled)
 *
 * Event registration, scheduling, wait queues, tracing, poll work,
 * userspace remain native Zephyr.
 */
...
int z_impl_k_poll_signal_raise(struct k_poll_signal *sig, int result)
{
	...
	/* Extract: dequeue first poll event (side effect: removes from list) */
	struct k_poll_event *poll_event =
		(struct k_poll_event *)sys_dlist_get(&sig->poll_events);

	/* Decide: Rust determines new signal state and action */
	struct gale_poll_signal_raise_decision d =
		gale_k_poll_signal_raise_decide(
			sig->signaled, result,
			poll_event != NULL ? 1U : 0U);

	/* Apply: write Rust's decision back to kernel state */
	sig->result = d.new_result;
	sig->signaled = d.new_signaled;

	if (d.action == GALE_POLL_ACTION_NO_EVENT) {
		k_spin_unlock(&lock, key);
	...
	/* SIGNAL_EVENT: wake the poller attached to this event */
