 ...
 *
 * Gale sched — full scheduler replacement.
 *
 * This is upstream kernel/sched.c with Gale decision struct integration
 * at the key scheduling decision points (should_preempt, next_up).
 * All 26 exported scheduler symbols are provided.
 */
...
/*
 * Variant of z_unpend_all() for callers that already hold
 * _sched_spinlock. Uses the non-locking unpend/ready helpers so the
 * scheduler lock is not recursively acquired. The woken threads are
 * readied but not rescheduled; the caller must arrange a reschedule
 * after releasing _sched_spinlock.
 *
 * Matches upstream Zephyr fix 9cef0da05c3 ("kernel: avoid recursive
 * scheduler lock in k_heap_free path").
 */
