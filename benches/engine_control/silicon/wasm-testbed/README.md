...
- **`arch/riscv/run_rv_funccheck.sh`** — RV32 **functional regression lane**: dissolves each leaf, compiles
  the actual `synth -b riscv` output, links it into a qemu harness and **asserts wasmtime-verified values
  across 10 vectors** (zero / positive / negative / ±127-saturation / signed-div-overflow / 4 table cells) —
  the per-axis input edges a const-CSE or liveness miscompile is most likely to break on only a subset of
