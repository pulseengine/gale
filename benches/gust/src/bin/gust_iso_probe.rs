//! gust-iso-probe — the dissolved ISOLATION CORE executed under qemu
//! (lm3s6965evb, real v7-M), reporting over semihosting.
//!
//! Same assertions as `gust_iso` (the Renode USART gate); this is the LOCAL
//! oracle, so the isolation core can be executed without a Renode install.
//! It is what first caught the `set-window` miscompile (gale#270).
//!
...
fn tx(s: &[u8]) {
    hprintln!("{}", core::str::from_utf8(s).unwrap_or("?").trim_end());
}

fn ok(cond: bool, good: &[u8], bad: &[u8]) {
    if !cond {
        unsafe { core::ptr::write_volatile(&raw mut FAILED, core::ptr::read_volatile(&raw const FAILED) + 1) };
    }
...
fn main() -> ! {
    unsafe {
        ...
        if failed == 0 {
            hprintln!("iso-probe: ALL CHECKS PASSED");
            debug::exit(debug::EXIT_SUCCESS);
        } else {
            hprintln!("iso-probe: {} CHECK(S) FAILED", failed);
