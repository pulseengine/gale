...
## THE UNION
    ...
    SMT-verified                       22
    declined but Rocq-qed             294        (read from proof-inventory.json)
    ------------------------------------------
    covered by the union              316  of 496   = 64%
    covered by NEITHER half           180  of 496   = 36%

**`REQ-OS-OBJVERIFY-001` is not met, and now has a number instead of an
adjective: 64% union coverage, 36% gap.** The gap is not arithmetic — it is
...
## 2026-08-27 — 51% -> 64%, because the matcher was guessing

Re-measured on synth **v0.60.0** against `artifacts/proof-inventory.json`
(`synth-proof-inventory-v1`, synth#1089), keyed on `entries[].constructor` with
**no theorem-name derivation anywhere**.

    applied 496   SMT-verified 22   Rocq-qed 294
    UNION 316 of 496 = 64%      gap 180 = 36%

### Why the old number was low — three failure modes, not one

Every previous figure here derived the theorem name from the rule kind by
CamelCase->snake_case. That lost **51 instances**:

| kind | derived | actual | mode |
|---|---|---|---|
| `BrIf` | `br_if_correct` | `brif_correct` | no separator |
| `I64LeS` | `i64_le_s_correct` | `i64_les_correct` | trailing sign joined |
| `I64GtS` | `i64_gt_s_correct` | `i64_gts_correct` | same |
| `I64ShrS` | `i64_shr_s_correct` | `i64_shrs_correct` | same |
| `I64ExtendI32S` | `..._correct` | `..._executes` | **not a `*_correct` theorem** |
| `I64ExtendI32U` | `..._correct` | `..._executes` | same |

The last two are the ones that settle the argument: they are discharged by
`*_executes` theorems, **outside the `*_correct` universe the matcher searched**.
No amount of fixing the naming rule would have found them. The number was derived
from a universe that had been assumed rather than checked.

An earlier revision of this document reported `BrIf` as the sole outlier and
declined to special-case it, arguing that hard-coding around an inconsistency
turns a matcher into a lookup table. Right conclusion, wrong reason: the real
defect was deriving a fact synth already knew.

### The gap is now fully accounted for

All 180 uncovered instances come back **"not in manifest"** — `End` 64,
`Block` 38, `Call` 31, `I32Load8U`/`I32Store8` 34, `Br` 7, `Unreachable` 6.
That is the genuine control-flow and byte-memory frontier, with nothing hiding
behind a naming mismatch. Previously some of the "gap" was measurement error.

### Standing caveat, unchanged

`proof-inventory.json` is generated by a second textual reader, not the Rocq
kernel — its own `_meta.honesty` says so. `_meta.cross_check.rocq_qed_total: 630`
agrees with what synth reports from `bazel test //coq:verify_proofs`. If the two
ever disagree, the kernel is the authority.
